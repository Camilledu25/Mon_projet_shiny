...
bilan_date<-bilan%>%filter(jour>=as.Date("2020-03-17"))
...
# 
# 
# temperature_only<-temperature_ord%>%select(jour,temperature)
...
# a=predict(model_knn)
# v<-data.frame(a)
# colnames(v) <- c("puissance_predite")
# join2<-cbind(temp_jfer_pos_vac, v)
# join2[1:10,]
# plot(puissance~jour,data=join2[1:10,],type="l",col="blue")
# par(new=TRUE)
# plot(puissance_predite~jour,data=join2[1:10,],type="l",col="red")
# class(join2$puissance_predite)
# class(join2$puissance)
# sum(join2$puissance_predite)-sum(join2$puissance)
# set.seed(1234)
...
server <- function(input, output) {
    ...
    get_table_knn<-reactive({
        bilan4<-bilan %>% 
            filter(categorie_client == input$segment) 
        join<- left_join(bilan4, temperature_ord)
        temperature_jfer <- mutate(join,
                                   jour_ferier = case_when(
                                       day(jour) == 25 & month(jour)==12 ~ 1,
                                       day(jour) == 31 & month(jour)==12 ~ 1,
        ...
        model_knn<-train(puissance~.-categorie_client-jour,data=temp_jfer_pos_vac,method='knn',trControl=ctrl,
                         preProcess=c("center","scale"),tuneGrid=param)
        predic<-predict(model_knn)
        dt<-data.frame(predic)
        colnames(dt) <- c("puissance_predite")
        join3<-cbind(temp_jfer_pos_vac, dt)
        table_final2<-mutate(join3,ecart=abs(puissance-puissance_predite))
        table_final1<-table_final2%>% 
            filter(jour >= input$obs) %>%
            filter(jour <= input$obs1)%>%
            select(jour,categorie_client,puissance,puissance_predite,ecart)
        table_final1
    ...
    output$courbe_realise_mod <- renderPlot({
        
        df1 <- get_table_knn() %>%
            select(jour, puissance,puissance_predite)%>%
    ...
    output$sommepred <- renderValueBox({
        out <-  get_table_knn()
        somme<-sum(out$puissance_predite)
        valueBox("Somme de la consommation prédite", somme
        )
    })
    ...
    output$diff <- renderValueBox({
        ...
        out2<-get_somme()
        outf<-abs(somme-out2)
        valueBox("Somme de la difference de consommation",outf
        )
    })
